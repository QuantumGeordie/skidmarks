module Skidmarks
  ...
  @crontab_data = []
  def self.crontab_data=(data)
    @crontab_data = data
  end

  def self.crontab_data
    @crontab_data
  end

  def self.generate_crontab_data_from_file
    ...
    @crontab_data = @crontab_data.join("\n")
  end

  def self.jobs(start_time, end_time)
    input_data = Skidmarks.crontab_data
